// packages/content-cli/src/cli/interactive/flows/note/publish.note.flow.interactive.cli.ts
...
export const runNotePublishFlow = async (
  state: InteractiveCliState,
): Promise<void> => {
  const slug = await text({
    message: "Note slug to publish",
  });
