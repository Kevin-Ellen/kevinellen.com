// packages/content-cli/src/content/notes/workspace.status.note.content.ts
...
type NoteWorkspaceStatusBucket = Readonly<{
...
export const getNoteWorkspaceStatus = async (
  env: ContentCliEnvironment,
): Promise<readonly NoteWorkspaceStatusBucket[]> => {
  ...
  return Promise.all(
    buckets.map(async (bucket) => {
      const directoryPath = path.join(NOTE_WORKSPACE_ROOT, bucket, env);
