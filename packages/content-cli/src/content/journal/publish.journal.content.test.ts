...
describe("runPublishJournalCommand", () => {
  ...
  it("validates draft, publishes photos, writes KV, and moves workspace", async () => {
    ...
    expect(mockedWriteKv).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ id: "journal:rye-house" }),
    );
  ...
  it("updates journalEntryFooter.updatedAt and leaves other modules untouched", async () => {
    const extraModule = {
      kind: "paragraph",
      content: [{ kind: "text", value: "ignore me" }],
    } as any;
    ...
    // Check journalEntryFooter updatedAt incremented
    const journalFooter = updatedFooter.find(
      (mod) => (mod as any).kind === "journalEntryFooter",
    );
    ...
    // Check paragraph module untouched
    const paragraphModule = updatedFooter.find(
      (mod) => (mod as any).kind === "paragraph",
    );
    expect(paragraphModule).toBe(extraModule);
  });
  ...
  it("handles undefined footer by using empty array", async () => {
    const draftNoFooter: AuthoredPublicPageDefinition = {
      ...
      content: {
        ...mockPage.content,
        footer: undefined, // trigger the fallback
      },
    };
    ...
    // Should still succeed and write KV
    expect(result.ok).toBe(true);
    expect(mockedWriteKv).toHaveBeenCalledWith(
      expect.anything(),
      "kv",
